...
const unitNames = computed(
  () => (employee.value?.units ?? []).map((u) => u.name).join(", ") || "—",
);
const companyNames = computed(
  () => (employee.value?.companies ?? []).map((c) => c.legalName).join(", ") || "—",
);
      ...
      <div class="space-y-6 lg:col-span-2">
        ...
        <div class="relative overflow-hidden rounded-2xl border border-mahir-border bg-white p-6 shadow-sm">
          ...
          <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
            <span class="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-2xl bg-mahir-primary-soft text-xl font-bold text-mahir-primary tracking-wide shadow-inner">
