...
import { useLeaveTypeFiltersStore } from "../stores/leaveTypeFilters.store";

// Layer logika tipe cuti: daftar paginated (reaktif terhadap filter & halaman).
export function useLeaveTypes() {
  const filters = useLeaveTypeFiltersStore();
  ...
  const data = computed(() => result.value?.listLeaveType?.data ?? null);
  const leaveTypes = computed(() => data.value?.results ?? []);
  ...
  return { leaveTypes, pagination, pageSize, loading, refetch, nextPage, prevPage, goToPage };
}
