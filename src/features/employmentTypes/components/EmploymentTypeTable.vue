<script setup>
import { IdentificationIcon, EyeIcon, PencilIcon, TrashIcon } from "@heroicons/vue/24/outline";
    ...
    <table class="w-full text-left text-sm">
      <thead>
        <tr class="border-b border-mahir-border text-xs uppercase tracking-wide text-slate-400">
          <th class="px-4 py-3 font-semibold">Tipe Kepegawaian</th>
          <th class="px-4 py-3 text-center font-semibold">Aksi</th>
        </tr>
      </thead>
      <tbody>
        <tr v-if="loading && !employmentTypes.length">
          <td colspan="2" class="px-4 py-8 text-center text-slate-400">Memuat data…</td>
        </tr>
        <tr v-else-if="!employmentTypes.length">
          <td colspan="2" class="px-4 py-8 text-center text-slate-400">
            Tidak ada tipe kepegawaian yang cocok.
          </td>
        </tr>
        <tr
