<script setup>
import { useLeaveBalanceTransactions } from "../composables/useLeaveBalanceTransactions";
import LeaveBalanceTransactionToolbar from "../components/LeaveBalanceTransactionToolbar.vue";
import LeaveBalanceTransactionTable from "../components/LeaveBalanceTransactionTable.vue";

const { transactions, pagination, loading, nextPage, prevPage } = useLeaveBalanceTransactions();
</script>
...
<template>
  ...
  <div class="overflow-hidden rounded-2xl border border-mahir-border bg-white">
    <div class="flex flex-wrap items-center justify-between gap-3 p-5">
      ...
      <LeaveBalanceTransactionToolbar />
    </div>

    <LeaveBalanceTransactionTable :transactions="transactions" :loading="loading" />

    <!-- Footer / pagination -->
    <div class="flex items-center justify-between border-t border-mahir-border px-5 py-3">
      <span class="text-[13px] text-mahir-muted"
        >Menampilkan {{ transactions.length }} dari {{ pagination.count }} mutasi</span
      >
