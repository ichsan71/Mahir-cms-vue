...
import { useLevelDetail } from "../composables/useLevelDetail";
...
const { level, loading } = useLevelDetail(id);

const employees = computed(() => level.value?.employees ?? []);
...
<template>
  ...
  >
    ...
    <p class="text-sm font-medium text-slate-500">Sinkronisasi data level...</p>
  </div>
  ...
  >
    ...
    <p class="text-sm font-medium text-slate-600">Data level tidak ditemukan</p>
    ...
    <div class="mb-6 relative overflow-hidden rounded-2xl border border-mahir-border bg-white p-6 shadow-sm">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="min-w-0 flex-1">
          <h1 class="text-xl font-bold tracking-tight text-slate-900">{{ level.name }}</h1>
          <p class="mt-1 text-sm font-medium text-slate-600">
            {{ employees.length }} karyawan pada level ini
          </p>
        </div>
      </div>
    </div>
          ...
          <tbody>
            <tr v-if="!employees.length">
              <td colspan="3" class="px-4 py-8 text-center text-slate-400">
                Belum ada karyawan pada level ini.
              </td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-3 text-slate-600">{{ emp.nik || "—" }}</td>
